...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemServiceImpl(ItemStorage itemStorage, UserStorage userStorage, BookingStorage bookingStorage,
                           CommentStorage commentStorage, ItemRequestStorage itemRequestStorage) {
    ...
    public ItemDto create(long userId, ItemWithRequestDto itemDto) {
        ...
        if (itemDto.getRequestId() != null) {
            Optional<ItemRequest> mayBeRequest = itemRequestStorage.findById(itemDto.getRequestId());
            if (mayBeRequest.isEmpty()) {
                throw new NotFoundException("Запроса с id = " + itemDto.getRequestId() + "не существует");
            }
            itemRequest = mayBeRequest.get();
        }
        log.debug("Предмет формата dto {}", itemDto);
        Item item = ItemMapper.mapToItemWithRequest(itemDto, user, itemRequest);
